...
parser.add_argument('--contractTransfer', help="Optional contract address of the token referenced by the quantization parameter (or none for ETH)")
parser.add_argument('--quantumTransfer', help="Optional quantization reference")
...
if args.quantumTransfer:
	...
	apdu = struct.pack(">BBBBB", 0xF0, 0x08, 0x00, 0x00, len(apduData)) + apduData
        try:
	    dongle.exchange(bytes(apdu))
        except:
            pass
