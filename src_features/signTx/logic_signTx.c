...
void finalizeParsing(bool direct) {
  ...
  if ((contractProvisioned == CONTRACT_STARKWARE_DEPOSIT_ETH) ||
      (contractProvisioned == CONTRACT_STARKWARE_DEPOSIT_TOKEN) ||
      (contractProvisioned == CONTRACT_STARKWARE_WITHDRAW)) {
    // For a deposit / withdrawal, check if the token ID is known or can't parse
    if (quantumSet) {
      ...
      if (os_memcmp(dataContext.tokenContext.data + 4, G_io_apdu_buffer + 100, 32) != 0) {
        ...
        PRINTF("Requested %.*H\n", 32, dataContext.tokenContext.data + 4);
