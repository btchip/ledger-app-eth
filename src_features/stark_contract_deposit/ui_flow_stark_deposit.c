...
void prepare_deposit_5() {
  ...
  tostring256(&amount, 10, G_io_apdu_buffer + 100, 100);
  strcpy(strings.common.fullAmount, ticker);
  adjustDecimals(G_io_apdu_buffer + 100, strlen(G_io_apdu_buffer + 100), strings.common.fullAmount + strlen(ticker), 50 - strlen(ticker), decimals);
}
